...
public class AdminController {
	...
	public String StudentInsert(@ModelAttribute("student") Student st,BindingResult errors,HttpServletRequest request) {
		...
		return "redirect: /TutorRent2/QuanLy/User.html";
	}
	...
	public String PostNotApplyUpdate(@PathVariable("postid") int id, HttpServletRequest request) {
		...
		try {
			...
			post.setApplyStatus(Boolean.parseBoolean(request.getParameter("applyStatus")));
			session.update(post);
			transaction.commit();
		} catch (Exception e) {
			transaction.rollback();
		...
		return "redirect: /TutorRent2/QuanLy/Post/PostNotApply.html";
	}
	...
	public String SubjectInsert(ModelMap model, @ModelAttribute("sub") Subject subject) {
		...
		return "redirect: /TutorRent2/QuanLy/MonHoc.html";
	}
